...
Future<Map<String, dynamic>> githubUpdate(
  ...
  final url =
      'https://api.github.com/repos/mainakgarai007/Music-hub/releases/latest';
...
/// Music-hub update API: checks only the Music-hub GitHub releases.
Future<Map<String, dynamic>> getAppUpdates() async {
  Map<String, dynamic> updates;
  try {
    updates = await githubUpdate();
  } catch (e) {
    log('Music-hub GitHub release check failed: $e', name: 'UpdaterTools');
    try {
      final packageInfo = await PackageInfo.fromPlatform();
      updates = {
        'results': false,
        'error': 'Failed to check Music-hub releases',
        'currVer': packageInfo.version,
      ...
      updates = {
        'results': false,
        'error': 'Failed to check Music-hub releases and local package info',
        'source': 'none',
      };
    }
  }
...
Future<String?> fetchChangelog(
    {Duration timeout = const Duration(seconds: 6)}) async {
  const changelogUrl =
      'https://mainakgarai007.github.io/Music-hub/CHANGELOG.md';
  try {
